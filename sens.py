...
def calculate_response(raw_fname, *, arc_fname, pixtable_fname, bias_fname, flat_fname,
                       output_dir='', pdf_fname='', order=8, interactive=False, dispaxis=2,
                       order_wl=4, order_bg=3, rectify_options={}, extraction_options={}, bg_options={}):
    """
    ...
    if star not in alfosc.standard_stars:
        msg.append(" [ERROR]  - No reference data found for this star  (TCS Target Name =  %s)" % hdr['TCSTGT'])
        output_msg = "\n".join(msg)
    ...
    msg.append("          - Median filtering the data points to remove outliers")
    ...
    hdu = fits.HDUList()
    hdu.append(tab)
    hdu.writeto(output_fname1D, overwrite=True)
    msg.append("          - Saved the response function as FITS table: %s" % output_fname1D)
    msg.append("")
    output_msg = "\n".join(msg)
    return output_fname1D, output_msg
